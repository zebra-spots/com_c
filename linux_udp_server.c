    ...
    else 
{
	...
	//printf("Buf len#1: %d \n", strlen(buf));
	//returnStatus = recvfrom(udpSocket, buf, (strlen(buf)+1), 0, (struct sockaddr *)&udpClient, &addrlen);
        ...
        if (returnStatus == -1) 
	{
	    ...
	    /*
            ...
            {
            fprintf(stderr, "Error was ECONNREFUSED");
            }
            else if (errno == EFAULT)
            {
            fprintf(stderr, "Error was EFAULT");
            }
            else if (errno == EINVAL)
            {
            fprintf(stderr, "Error was EINVAL");
            }
            else if (errno == ENOTCONN)
            {
            fprintf(stderr, "Error was ENOTCONN");
            }
            */
        ...
        else 
	{
            ...
            returnStatus = sendto(udpSocket, buf, (strlen(buf)+1), 0, (struct sockaddr *)&udpClient, addrlen);
